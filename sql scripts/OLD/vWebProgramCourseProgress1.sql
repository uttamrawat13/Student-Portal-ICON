...
/****** Object:  View [dbo].[vWebProgramCourseProgress1]    Script Date: 03/24/2011 15:59:13 ******/
SET ANSI_NULLS ON
GO
...
FROM         CourseOfferingStudent AS COS INNER JOIN
                      ...
                      Course AS C ON CO.CourseNo = C.CourseNo INNER JOIN
                      Grade AS G ON COS.GradeNo = G.GradeNo
WHERE     G.UnitsInProgress = 'Y'
UNION ALL
SELECT     S.StudentNo, 'Required' AS ReportGroup, C.CourseNo, C.CourseCode, C.CourseDescription, C.Units
FROM         Students AS S INNER JOIN
                      ProgramCourse AS PC ON S.ProgramNo = PC.ProgramNo INNER JOIN
                      Course AS C ON PC.CourseNo = C.CourseNo
