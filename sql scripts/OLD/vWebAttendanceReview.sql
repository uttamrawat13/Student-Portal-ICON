...
/****** Object:  View [dbo].[vWebAttendanceReview]    Script Date: 03/24/2011 15:57:57 ******/
SET ANSI_NULLS ON
GO
...
AS
SELECT     COAS.StudentNo, S.FirstName + ' ' + S.LastName AS Student, C.CourseCode + '  (' + CO.[Session] + CAST(CO.SessionNo AS char(2)) + ')' AS Course, T .TermBeginDate, 
                      COS.ClassDate, COS.ClassStartTime, COS.ClassEndTime, COS.ClassHours, COS.Complete, COAS.AttendanceHours, COAS.AttendanceCode, C.CourseDescription, 
...
FROM         CourseOfferingAttendanceScheduled AS COAS INNER JOIN
                      Students AS S ON COAS.StudentNo = S.StudentNo INNER JOIN
...
UNION ALL
SELECT     COANS.StudentNo, S.FirstName + ' ' + S.LastName AS Student, C.CourseCode + '  (' + CO.[Session] + CAST(CO.SessionNo AS char(2)) + ')' AS Course, 
                      T .TermBeginDate, COANS.ClassDate, COANS.ClassStartTime, COANS.ClassEndTime, ISNULL(COANS.ScheduledHours, 0) AS ClassHours, '' AS Complete, 
                      COANS.AttendanceHours, COANS.AttendanceCode, C.CourseDescription, ISNULL(COANS.AttendanceHours, 0) * AC.Present AS AttendedAdj, 
                      ISNULL(COANS.ScheduledHours, 0) * AC.Scheduled AS ScheduledAdj
FROM         CourseOfferingAttendanceNotScheduled AS COANS INNER JOIN
                      Students AS S ON COANS.StudentNo = S.StudentNo INNER JOIN
